...
const PRLHome = ({ navigation }) => {
  const [loading, setLoading] = useState(true);
  const [refreshing, setRefreshing] = useState(false);
  const [userData, setUserData] = useState(null);
  const [activeTab, setActiveTab] = useState('overview'); // overview, monitoring, ratings
  ...
  const loadData = async () => {
    try {
      ...
      // Get lecturers
      const lecturersQuery = query(collection(db, 'users'), where('role', '==', 'lecturer'), where('faculty', '==', userStream));
      ...
      // Get students
      const studentsQuery = query(collection(db, 'users'), where('role', '==', 'student'), where('faculty', '==', userStream));
      ...
      console.error('Error loading PRL data:', error);
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <View style={styles.container}>
      <Header title="PRL Dashboard" navigation={navigation} showLogout={true} />
      ...
      >
        ...
        <View style={styles.profileSection}>
          ...
          <View style={styles.profileInfo}>
            <Text style={styles.userName}>{userData?.name || 'Principal Lecturer'}</Text>
            <Text style={styles.userStream}>Stream: {userData?.stream || userData?.faculty || 'ICT'}</Text>
            <View style={styles.roleBadge}>
              <Icon name="award" size={12} color={COLORS.navy} />
              <Text style={styles.roleText}>Principal Lecturer</Text>
            </View>
          </View>
        </View>
        ...
        <View style={styles.statsRow}>
          <TouchableOpacity style={styles.statCard} onPress={() => navigation.navigate('PRLCourses')}>
            <RoundContainer glow>
        ...
        <RoundContainer glow style={styles.ratingSummaryCard}>
          <Text style={styles.ratingSummaryTitle}>Overall Stream Rating</Text>
        ...
        >
          ...
          <Text style={styles.reportsButtonText}>Review Reports ({reportsCount})</Text>
              ...
              return (
                <RoundContainer key={index} glow style={styles.studentCard}>
                  <View style={styles.studentHeader}>
                    ...
                    <Text style={[styles.attendancePercent, { color: stats.percentage >= 75 ? COLORS.success : COLORS.danger }]}>
                      {stats.percentage}%
                    </Text>
                  </View>
                </RoundContainer>
              ...
              return (
                <RoundContainer key={index} glow style={styles.ratingCard}>
                  <View style={styles.ratingHeader}>
                    <View>
                      <Text style={styles.ratingLecturerName}>{lecturer.name}</Text>
                      <Text style={styles.ratingDept}>{lecturer.department}</Text>
                    </View>
...
const styles = StyleSheet.create({
  ...
  userStream: { fontSize: 13, color: COLORS.textLight, marginTop: 2 },
