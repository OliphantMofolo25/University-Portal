...
const PLCourses = ({ navigation }) => {
    ...
    try {
      ...
      if (!currentCourses.includes(selectedCourse.id)) {
        await updateDoc(doc(db, 'users', selectedLecturer.id), {
          ...
          assignedCourses: [...(selectedLecturer.assignedCourses || []), { 
            id: selectedCourse.id, 
            name: selectedCourse.name, 
            code: selectedCourse.code 
          }]
        });
        Alert.alert('Success', `Course "${selectedCourse.name}" assigned to ${selectedLecturer.name}`);
        setAssignModalVisible(false);
        setSelectedCourse(null);
        setSelectedLecturer(null);
        loadData();
      } else {
        Alert.alert('Info', 'Lecturer is already assigned to this course');
      }
    } catch (error) {
  ...
  const handleRemoveLecturer = async (lecturer, course) => {
    Alert.alert(
      'Confirm Remove',
      `Remove ${lecturer.name} from ${course.name}?`,
      [
        { text: 'Cancel', style: 'cancel' },
        {
          text: 'Remove',
          ...
          onPress: async () => {
            try {
              const updatedCourseIds = (lecturer.courseIds || []).filter(id => id !== course.id);
              const updatedAssignedCourses = (lecturer.assignedCourses || []).filter(c => c.id !== course.id);
              await updateDoc(doc(db, 'users', lecturer.id), {
                courseIds: updatedCourseIds,
                assignedCourses: updatedAssignedCourses
              });
              Alert.alert('Success', `Course removed from ${lecturer.name}`);
              loadData();
            } catch (error) {
  ...
  const getAssignedLecturers = (courseId) => {
    return lecturers.filter(lecturer => lecturer.courseIds?.includes(courseId));
  };
      ...
      <ScrollView style={styles.content} refreshControl={<RefreshControl refreshing={refreshing} onRefresh={onRefresh} />}>
        ...
        {courses.length === 0 ? (
          <RoundContainer glow style={styles.emptyContainer}>
            ...
            <Text style={styles.emptySubtext}>Tap "Add Course" to create one</Text>
          </RoundContainer>
        ) : (
            ...
            return (
              <RoundContainer key={index} glow style={styles.courseCard}>
                ...
                <View style={styles.assignedSection}>
                  <Text style={styles.assignedTitle}>Assigned Lecturers ({assignedLecturers.length})</Text>
                  {assignedLecturers.length === 0 ? (
                    <Text style={styles.noAssignedText}>No lecturers assigned</Text>
                  ) : (
                    assignedLecturers.map((lecturer, idx) => (
                      <View key={idx} style={styles.assignedItem}>
                        <Icon name="user-check" size={14} color={COLORS.success} />
                        <Text style={styles.assignedName}>{lecturer.name}</Text>
                        <TouchableOpacity onPress={() => handleRemoveLecturer(lecturer, course)}>
      ...
      <Modal visible={modalVisible} animationType="slide" transparent>
        <View style={styles.modalOverlay}>
          <View style={styles.modalContent}>
            ...
            <ScrollView showsVerticalScrollIndicator={false}>
              ...
              <View style={styles.facultyPicker}>
                {faculties.map((faculty) => (
                  <TouchableOpacity
                    key={faculty}
                    style={[styles.facultyOption, formData.faculty === faculty && styles.facultyOptionSelected]}
                    onPress={() => setFormData({ ...formData, faculty })}
                  >
                    <Text style={[styles.facultyOptionText, formData.faculty === faculty && styles.facultyOptionTextSelected]}>
                      {faculty}
                    </Text>
                  </TouchableOpacity>
                ))}
            ...
            <ActionButton
              title={editingCourse ? 'Update' : 'Save'}
          ...
          <View style={styles.modalContent}>
            ...
            <ScrollView style={styles.pickerList}>
              {courses.map((course) => (
                <TouchableOpacity
                  key={course.id}
                  style={[styles.pickerOption, selectedCourse?.id === course.id && styles.pickerOptionSelected]}
                  onPress={() => setSelectedCourse(course)}
                >
                  <Text style={[styles.pickerOptionText, selectedCourse?.id === course.id && styles.pickerOptionTextSelected]}>
            ...
            <ScrollView style={styles.pickerList}>
              {lecturers.map((lecturer) => (
                <TouchableOpacity
                  key={lecturer.id}
                  style={[styles.pickerOption, selectedLecturer?.id === lecturer.id && styles.pickerOptionSelected]}
                  onPress={() => setSelectedLecturer(lecturer)}
                >
                  <Text style={[styles.pickerOptionText, selectedLecturer?.id === lecturer.id && styles.pickerOptionTextSelected]}>
...
const styles = StyleSheet.create({
  ...
  courseCredits: { fontSize: 13, color: COLORS.textLight, marginBottom: 4 },
  courseDescription: { fontSize: 12, color: COLORS.textDark, marginTop: 4 },
  
  assignedSection: { marginTop: 10, paddingTop: 8, borderTopWidth: 1, borderTopColor: COLORS.border },
  assignedTitle: { fontSize: 13, fontWeight: '600', color: COLORS.textDark, marginBottom: 6 },
  assignedItem: { flexDirection: 'row', alignItems: 'center', gap: 8, marginBottom: 6 },
  assignedName: { fontSize: 13, color: COLORS.textDark, flex: 1 },
  noAssignedText: { fontSize: 12, color: COLORS.textLight, fontStyle: 'italic' },
  ...
  assignSubmitButton: { marginTop: 20, marginBottom: 10 },
});
